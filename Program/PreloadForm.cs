...
namespace Program
{
    ...
    {
        private static readonly string BASE_PATH = "https://raw.githubusercontent.com/kanohban/StarSaviorArcanaDB/refs/heads/gh-pages/";
        ...
        private async void StartLoad()
        {
            ...
            const int total = 5;
            UpdateUI(1, total, "새로운 선택지가 있는지 확인하는중...", 0, "Fetch Github Server...");

            await Task.Delay(100);

            string cards = await FetchStringWithProgressAsync(client, $"{BASE_PATH}data/cards.json", progress =>
            {
                UpdateUI(1, total, "새로운 선택지가 있는지 확인하는중...", (int)(progress * 33.0f), "Fetch cards.json");
            });

            await Task.Delay(50);
            string journeyData = await FetchStringWithProgressAsync(client, $"{BASE_PATH}data/journey_data.json", progress =>
            {
                UpdateUI(1, total, "새로운 선택지가 있는지 확인하는중...", (int)(progress * 33.0f) + 33, "Fetch journey_data.json");
            });

            await Task.Delay(50);
            string potentials = await FetchStringWithProgressAsync(client, $"{BASE_PATH}data/potentials.json", progress =>
            {
                UpdateUI(1, total, "새로운 선택지가 있는지 확인하는중...", (int)(progress * 33.0f) + 33, "Fetch potentials.json");
            });

            await Task.Delay(50);
            UpdateUI(1, total, "새로운 선택지가 있는지 확인하는중...", 100, "데이터 교체 여부 확인중");
            ...
            if (saveJourney || saveCards || savePotentials)
            {
                UpdateUI(2, total, "새로운 선택지 데이터를 저장하는중...", 0, "Save Server Data...");
                ...
                await Task.Delay(50);
                UpdateUI(2, total, "새로운 선택지 데이터를 저장하는중...", 100, "Save Server Data...");
            }
            await Task.Delay(50);

            UpdateUI(3, total, "선택지 데이터를 불러오는중...", 0, "선택지 데이터 파일을 읽는중...");
            await EventLoader.Load();
            await Task.Delay(50);
            UpdateUI(3, total, "선택지 데이터를 불러오는중...", 100, "선택지 데이터 파일을 읽는중...");
            await Task.Delay(50);

            UpdateUI(4, total, "아르카나 카드 이미지가 있는지 확인하는중...", 0, "대기중...");
            ...
            foreach(var c in card)
            {
                ...
                UpdateUI(4, total, "아르카나 카드 이미지가 있는지 확인하는중...", (int)(idx * 100.0f / card.Count), $"Check File Exists : \"{c.Image}\"");

                string saveImgPath = $"./{c.Image}";

                if (!File.Exists(saveImgPath))
                {
                    byte[] imageBytes = await FetchBytesWithProgressAsync(client, $"{BASE_PATH}{c.Image}");
                    File.WriteAllBytes(saveImgPath, imageBytes);
                }
                ++idx;
            }
            UpdateUI(4, total, "아르카나 카드 이미지가 있는지 확인하는중...", 100, "완료");

            await Task.Delay(50);

            UpdateUI(5, total, "업데이트 완료", 100, "완료");
